...
Inveon tracks visitor sessions and provides consultancy based on their analysis of a business' website. The firm would like to know what features belong to customers who result in a sale for their clients. We are interested in finding out how much these people spend on the site, when they browse and buy, where they're located, and what technologies they use to explore a company's online store. We want to see the common themes/characteristic of those household, look at the factors that affect income level the most, and put resources into areas that can help increase their income level.
...
For this first part, we just want to do some basic modeling of all available features, and probe for the most effective features. Because, we're using all features, we will not be employing a random forest model in this section. That means right now we'll focus on:
- logistic regression
...
```{r step_model_full, warning=FALSE, message=FALSE}
...
step_predictions_logical_full <- factor(ifelse(step_predictions_full == "0", FALSE, TRUE), levels = c(FALSE, TRUE))
...
```{r step_model_pruned, warning=FALSE, message=FALSE}
...
# Converting the format of predicted binary values
step_predictions_logical <- factor(ifelse(step_predictions == "0", FALSE, TRUE), levels = c(FALSE, TRUE))
...
specificity_matrix <- data.frame(
  ...
  Full_Model = c(
    ...
    step_cm_full$byClass['Specificity'],
...
sensitivity_matrix <- data.frame(
  ...
  Full_Model = c(
    ...
    step_cm_full$byClass['Sensitivity'],
