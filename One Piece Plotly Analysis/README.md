...
- **Spoiler Warning**: I won't extensively cover the synopsis of the *One Piece* series here, but some insights regarding episode titles and narrative descriptions will inevitably be revealed. So, if you're sensitive to spoilers for this series, consider this your warning.
- GitHub, where I primarily showcase my data science work, does not support rendering Plotly graphs in Python or Jupyter Notebooks. For this project, [I recommend viewing a duplicated version on Kaggle](https://www.kaggle.com/code/bryantjay/one-piece-plotly-analysis). This version fully showcases the dynamic functionalities of each graph, which is a major reason for using Plotly in the first place.

## Introduction

Eiichiro Oda's *One Piece* is one of the largest media franchises to come out of Japan. The story follows Monkey D. Luffy, a young pirate with rubber-like abilities and a signature straw hat, who sets out on a journey to find the legendary treasure "One Piece" and become the King of the Pirates. This simple premise has kept fans of the series hooked and eagerly awaiting a conclusion for nearly three decades.
...
First published in 1997 as a manga in *Weekly Shonen Jump*, *One Piece* has captivated readers worldwide with its rich storytelling and colorful characters. Toei Animation's anime adaptation began airing in 1999, bringing the epic odyssey to television screens across Japan. Over time, the *One Piece* franchise has evolved into a global sensation. An early English-language dub was handled by 4Kids Entertainment during the mid-2000s, but this version faced criticism for heavy censorship and narrative rewrites. Funimation acquired the English localization rights in 2007 and began producing a more faithful English version. This is the version that would go on to air on Toonami, Crunchyroll, and Netflix for most of the series's history.

As of 2025, the *One Piece* manga is [the best-selling manga in history](https://en.wikipedia.org/wiki/List_of_best-selling_manga#At_least_100_million_copies_and_above), having even surpassed the sales of other international comic book series, making it [the best-selling comic book in history](https://en.wikipedia.org/wiki/List_of_best-selling_comic_series#Collected_comic_book_volumes). The television show has firmly cemented its influence among other top anime series, such as *Dragon Ball* and *Pokémon*. As the show approaches its final story arcs, it has experienced a renewed surge in popularity. Additionally, the release of *One Piece*'s live-action Netflix adaptation and the announcement of a future remake under Wit Studio (of *Attack on Titan* acclaim) have further sparked mainstream interest, introducing the franchise to new audiences and reigniting enthusiasm among existing fans.

This data visualization project intends to explore the *One Piece* anime throughout its production history and various story arcs. It will focus on the anime but will briefly use some pre-included manga data to generate insights related to the show's development. We will use a combination of Jupyter Notebooks and Plotly for Python to create dynamic, information-rich charts. Plotly's interactive capabilities make it a great choice for others to easily explore the dataset. This Jupyter Notebook includes direct HTML links to copies of the data sources hosted on my GitHub page and only requires common package installations like Pandas and BeautifulSoup to run on your own machine. However, for users who lack the ability to use Python and/or the relevant packages, I've included a version on Kaggle that is runnable without any coding knowledge whatsoever.


## Importing the Data

This data workflow will primarily use the Python version of Plotly; we'll be working with the `graph_objects` and `express` extensions, and optionally the `io` extension for exporting static images of the plots. We'll also be employing `BeautifulSoup` along with a few other common data science packages.

I've established a URL source on [my GitHub page](https://github.com/bryantjay/Portfolio/tree/main/One%20Piece%20Plotly%20Analysis/source_files) to host this data online in raw CSV format. The data itself originally comes from two Kaggle sources: [a dataset on One Piece episode ratings](https://www.kaggle.com/datasets/aditya2803/one-piece-anime) and [a dataset on *One Piece* story arc summaries](https://www.kaggle.com/datasets/tarundalal/one-piece-arcs). We will also be scraping updated television release dates and English-translated titles from an [online source](https://listfist.com/list-of-one-piece-anime-episodes); a copy of this scraped data is also hosted on my GitHub in case the webpage or dataset becomes inaccessible.

We'll start by reading in the episode data as a Pandas DataFrame called `df`, which will be the main dataframe we focus on.
...
The user on Kaggle who uploaded this dataset states that the data came from [this website](https://www.ratingraph.com/tv-shows/one-piece-ratings-17673/) on TV ratings. Fields such as "rank" "trend" and "total_votes" primarily reflect *One Piece* episode metrics on that site, relative to other television shows.

We can see that the index includes a duplicated "unnamed" ID column. Additionally, while the first six episodes are labeled in English, most of the subsequent episode titles are in their original Japanese. We want to resolve this mixed-language discrepancy and ideally use consistent English translations.

There's an up-to-date and well-formatted table of episode numbers, English episode titles, and exact broadcast dates for all *One Piece* episodes at [the following link](https://listfist.com/list-of-one-piece-anime-episodes). The table on this page is easily scrapable using `requests` and `BeautifulSoup`. The data will first be read into three Python lists—one list per column.

Between the subtitled translations, the 4Kids dub, and the Funimation dub, there are several possible English versions of each episode title. I believe this webpage relies mostly on either the Funimation dub or a direct translation from the original Japanese; either of these is preferable for this project. As a bonus, we also get full broadcast dates, rather than just the year of release, for each episode’s *original* (i.e., Japanese) air date.
...
The data has successfully been scraped and is now ready to be added to the main dataframe.
...
With the proper episode names added, we can drop the old column containing mixed-language episode titles. As a precaution, the string `strip()` method will be applied to trim any potential leading or trailing whitespace. The original title case formatting will be preserved.

At this point, other data inconsistencies can be cleaned, and data attributes can be properly examined using the `info()` and `describe()` methods. The 'total_votes' column needs to be cleaned of commas and converted to a numeric datatype. The 'release_date' column also needs to be converted to a datetime datatype. Before this can be done, an erroneous period in row position 863 must be corrected.
...
```
    ...
    9   9  41,829     4       1       10   
                                                    ...
                                                    name  start  total_votes  \
    0  I'm Luffy! The Man Who Will Become the Pirate ...   1999        647.0   
    1  Enter The Great Swordsman! Pirate Hunter Roron...   1999        473.0   
    2  Morgan vs. Luffy! Who's This Mysterious Beauti...   1999        428.0   
    3       Luffy's Past! The Red-Haired Shanks Appears!   1999        449.0   
    4  Fear, Mysterious Power! Pirate Clown Captain B...   1999        370.0   
    5  Desperate Situation! Beast Tamer Mohji vs. Luffy!   1999        364.0   
    6  Grand Duel! Zoro the Swordsman vs. Cabaji the ...   1999        344.0   
    7  Who Will Win? Showdown Between the True Powers...   1999        335.0   
    8                      Honorable Liar? Captain Usopp   2000        327.0   
    9  The World's Strongest Weirdo! Jango the Hypnot...   2000        314.0   
    
       average_rating release_date  
    0             7.6   1999-10-20  
    1             7.8   1999-11-17  
    2             7.7   1999-11-24  
    3             8.1   1999-12-08  
    ...
    None 
                    ...
                    id    rank trend  season     episode  \
    count   958.000000     958   958   958.0  958.000000   
    unique         NaN     958    34     NaN         NaN   
    top            NaN  24,129     -     NaN         NaN   
    freq           NaN       1   374     NaN         NaN   
    mean    478.500000     NaN   NaN     1.0  479.500000   
    min       0.000000     NaN   NaN     1.0    1.000000   
    25%     239.250000     NaN   NaN     1.0  240.250000   
    50%     478.500000     NaN   NaN     1.0  479.500000   
    75%     717.750000     NaN   NaN     1.0  718.750000   
    max     957.000000     NaN   NaN     1.0  958.000000   
    std     276.695079     NaN   NaN     0.0  276.695079   
    
                                                         name        start  \
    count                                                 958   958.000000   
    unique                                                958          NaN   
    top     I'm Luffy! The Man Who Will Become the Pirate ...          NaN   
    freq                                                    1          NaN   
    mean                                                  NaN  2010.231733   
    min                                                   NaN  1999.000000   
    25%                                                   NaN  2005.000000   
    50%                                                   NaN  2010.000000   
    75%                                                   NaN  2015.000000   
    ...
    std                                                   NaN     6.049997   
            ...
            total_votes  average_rating                   release_date  
    count    958.000000      958.000000                            958  
    unique          NaN             NaN                            NaN  
    top             NaN             NaN                            NaN  
    freq            NaN             NaN                            NaN  
    mean     152.927975        7.796555  2010-09-26 08:14:31.816283904  
    min       70.000000        5.600000            1999-10-20 00:00:00  
    25%      117.000000        7.500000            2005-08-08 18:00:00  
    50%      132.000000        7.800000            2010-12-15 12:00:00  
    75%      157.750000        8.200000            2015-11-20 06:00:00  
    max     2862.000000        9.600000            2021-01-17 00:00:00  
    std      108.653063        0.589670                            NaN  
    

### Import Data on Story Arcs

All episode numbers are correct, but each episode is currently listed as being part of "Season 1." While this is technically true, as there are no explicit "seasons," it doesn’t provide much insight. *One Piece* (like many other anime series) is often broken down into story "arcs" rather than explicit "seasons." We'll categorize episodes based on their story arcs as well.

Once again, I'm hosting a copy of this dataset on GitHub, but it originally comes from [here](https://www.kaggle.com/datasets/tarundalal/one-piece-arcs).
...
Here's what the first few rows of the `arcs` dataset initially look like:
...
You can see that this information focuses on the prevalence of story arcs within both the manga (print) and anime (television) formats. You might notice that the values in the "TotalMinutes" field are simply the product of "TotalEpisodes" multiplied by 24.

The information on manga content is out of scope, as this analysis will focus on the television series. However, there is one important use for the manga fields "Start onChapter" and/or "TotalChapters". You'll notice that some instances of these fields have a value of zero. This occurs when there are anime story arcs that do not appear in the original manga. This kind of content is often referred to as "filler."
...
- Manga cover page stories that were expanded upon to create full episodes (e.g. the "Buggy Side Story Arc" and the "Koby and Helmeppo" training arc).
...
I want to take a look at the existence of filler arcs and episodes within the show, so let's do this with the creation of a binary categorical column, called "Function". Essentially, if the arc exists in the manga, we'll label it "canon". If it does not exist, we'll label it "filler" or non-canon.
...
I mentioned earlier that this series is not easily broken down into explicit "seasons", as the show has been undergoing weekly development and release schedules year-round for 25 years — up until recently (the animation received a three-month break between 2024 and 2025). We were able to break everything down into story arcs, but this grouping is still not perfect. There are around 50 different story arcs in the anime, many of which are only 3-4 episodes long. These co-exist with larger story arcs that can be dozens or even hundreds of episodes long (the final length of the Wano arc is just over 200 episodes!). Generally, later story arcs tend to last much, much longer than the earlier ones.

As the series has been going on for so long, breaking down all the episodes into their story arcs doesn't work as well as it might for most other shorter-running series. It's noticeable that the number of arcs builds up over time. However, there are still large narrative chunks of the story that strongly pertain to one another in large groupings. "East Blue" effectively acts as the introduction and prologue to the rest of the series. "Ennies Lobby" is often seen as the second half of "Water Seven". Similar relationships exist between "Impel Down" and "Marineford", along with other parts of the story.

Toei Animation also takes note of this, and has grouped and marketed cohesive sections of the series into larger collections. Instead of seasonal releases, multiple story arcs and various one-shots have been grouped and marketed as larger box sets, affectionately dubbed ["Eternal Logs"](https://store.toei-anim.co.jp/shop/g/gEYXA-13885-7) in reference to an item from the show. [We'll use these collections as our grouping criteria for tying together related story arcs](https://onepiece.fandom.com/wiki/Home_Video_Releases).

Some collections ("Dressrosa", "Whole Cake Island", and "Wano") are large enough that their Eternal Log sets have been further split into multiple parts. We're going to keep these parts grouped together. I'm also going to alias these larger groupings of story arcs as "sagas", in tradition with the first collection, the "East Blue Saga". There is some disagreement and ambiguity as to which arcs pertain to which sagas, which is why I'm relying on the Eternal Log releases as a guide (so, although one could argue that "Punk Hazard should be included with Dressrosa", or "Sabaody should be grouped with the rest of the 'Summit War' arcs", I'm not relying on those groupings here).
...
At this point, we can drop the unnecessary columns in the `arcs` dataframe. We have concluded that we don't need any info on manga chapters, since we won't be looking at manga data. The column 'TotalMinutes(avg 24)' also seems to be of limited use.
...
##### Merging `arcs` with the original dataframe

The improved dataframe with all the different story arcs looks great! Now, we need to incorporate it into the rest of our data. In order to merge the `arcs` dataframe with our original `df` dataframe, we will need to define which episode ranges belong to which arcs. We can do this by marking the first and last episodes of each story arc in the `arcs` dataframe, along with a corresponding arc index. Then, we can create and map a custom function to fill the episode ranges of the `df` dataframe with their respective arc indexes.
...
- The 'rank' and 'trend' fields deal with how the One Piece episodes trend among episodes from other shows on the website where this data was collected, which is outside the scope of this EDA project.
...
The data looks pretty good now! I should note here that, while this dataset contains only the first 958 episodes of the anime (about midway through Wano), I'm currently writing this during the show's extended hiatus in early 2025 (about 1120 episodes in, during Egghead Island). So, it should be noted that this version of the data is approximately ~4 years out of date.
...
We also need to correct a couple of story arc labeling errors in Impel Down and Wano, which occurred following movie filler arcs. Additionally, there seems to be a data entry error for 'total_votes' in the last instance. This value is around 10 times higher than nearby preceding values, and to my knowledge, this is not concerning a popular or well-known episode of the show. This value will be imputed to a more reasonable figure of 280 — about the median of nearby instances.
...
df.loc[df.index[956], 'total_votes'] = 280
print(df.iloc[956])
```
...
If you are viewing without the ability to render Plotly graphs, here's what the chart looks like:
...
There's a much greater overlap between the canon and filler episodes for this metric. It also sees much greater rightward skewness in its distribution, with a few canon episodes representing the most outlying points. It doesn't seem like there is a whole lot of differentiation between 'Function' levels for this metric.
...
What we've essentially created is a timeline of the *One Piece* anime series that is absolutely packed with information to explore. We can see how overall ratings remain relatively consistent throughout the show's history, fluctuating around an 8 out of 10. There is a steady growth in ratings as the story develops during Ennies Lobby, Marineford, Dressrosa, Whole Cake Island, and Wano. We also see substantial dips in ratings during the Fishman Island and Punk Hazard arcs, as well as most "post-arc" time spans when an abundance of filler content is released (notice the orange segments that follow the conclusions of major story arcs like Alabasta, Skypeia, and Ennies Lobby).

Building on this point, the distinction in average ratings between filler and non-filler material is more transparent in this visual. One can observe sharp downward spikes at any point in the story where filler material is inserted. Particularly notable is the section of anime episodes 196-226, which consists of 30 straight episodes of almost exclusively filler content. The only canon material in this stretch is half* of the Long Ring Long Land arc, which itself is sometimes treated as a very filler-ish canon arc (* 3 out of the 6 competitions in this arc are anime-only). There are also numerous points where filler content is inserted throughout the Ennies Lobby arc. As an aside, in my opinion, these light-hearted filler episodes are very jarring when they're dropped in the middle of the more climactic and serious story points during Ennies Lobby; this is likely why there is so much fluctuation in ratings between episodes 278 and 325.
...
We do start to see increased wave height in the ratings fluctuations occurring towards the tail of the graph, from around episode 800 onwards. I'm curious to know what exactly is causing this, as only a small number of these peaks and dips are represented by climax and filler episodes, respectively. Many of the later seasons are much longer, and some storyline components that occur during them can sometimes be criticized for "dragging on"; it's possible that these fluctuations could be related to the perceived pacing of the story. Other potential reasons could include the boost in Western interest in the show toward the end of the 2010s, or the increased movement of fanbases towards established social media sites in the 2010s. Changes in mainstream popularity or fan shifts to [algorithm-driven platforms](https://medium.com/the-guild-association/hacking-the-hive-mind-af0601ca3206) could potentially drive ["hive-minded" tendencies](https://www.reddit.com/r/TheoryOfReddit/comments/1fkatpj/reddits_hive_mind_mentality_how_it_brings_out_the/) within a given community.
...
![Ratings Timeline of One Piece series, colored by canon and filler episodes](https://github.com/bryantjay/Portfolio/blob/main/One%20Piece%20Plotly%20Analysis/visualizations/op_ratings_over_time.png?raw=true)
...
Continuing from ratings over time, let's now take a look at how the number of user votes changes. Instead of using release date, I'm going to organize this next chart by episode number. This highlights an important quirk in the standardized nature of the series: there is nearly no extensive deviation in Toei Animation's weekly release schedule over the past 25 years. [Only recently did the anime undergo the first extended hiatus in its history](https://gamerant.com/one-piece-why-anime-hiatus-explained/), a break lasting six months. Up until now, the series has maintained an impressive production schedule of releasing one episode per week, every week, for as long as it has been in production. If one looks closely at the previous timeline, they might notice a few occurrences where one week was skipped, or another week had a bonus episode release; for the most part, the one-episode-per-week tradition is consistent, meaning that the episode numbers are closely synchronized with release dates.

We're going to again color this next plot based on the presence of canon and filler episodes within the series. Luckily, we can re-use the same line segment dataframe from earlier, and *also* the structure of the Plotly code used in the time series plot. Essentially, we only need to adjust our X and Y references, and any relevant titles and axis labels. The annotations are going to be re-customized for this chart, as their positions will need to be shifted slightly. We won't be using a LOESS trend line in this plot, as there isn't really a need for it.
...
This secondary line chart gives a better impression of reviewer activity relative to each part of the story. We mostly only notice spikes in user activity around memorable or climactic moments in the story. We don't see the same dips for filler episodes as we saw in the ratings chart, though it does seem like voter levels during filler gaps may be *marginally* lower than those of surrounding episodes (like a *micro*-decrease).

There is far more user activity toward both ends of the chart. Generally, it does seem that earlier episodes — those before around episode ~130 — tend to have more voter activity on average; particularly, the earlier the episode is before this point, the higher the activity level. I would guess that a lot of this elevated activity is due to nostalgia and/or early viewers who either stopped watching or stopped reviewing later episodes. It should be noted that both [4Kids](https://www.animenewsnetwork.com/news/2006-12-06/4kids-cancels-one-piece-production) and [Toonami](https://www.animenewsnetwork.com/news/2008-03-29/cartoon-network-has-no-plans-for-one-piece-return) stopped or halted English localizations of the series between the Alabasta and Skypeia arcs, before Funimation later resumed work on their dubs in 2011. (If you were a 2007 American viewer like me, you might remember them climbing this waterfall-thing to enter Skypeia, and never getting to see the rest. Sooo, that's why.)

The tail of the chart also sees more activity, and again, this is likely due to increasing audiences in recent years. We also start to see far more upward spikes in voter levels toward the end of the chart; this is likely related to the large ratings fluctuations observed in the previous charts.
...
For the most part, we see that ratings and total votes are largely neutral and independent for episodes with low to mid ratings, but there is more of an increasing relationship for instances at the higher end of episode ratings (8 and above). Most episodes seem to float around the median line, which itself has a log-like quality to it. The high-rating, high-vote scatter points seen toward the right side of the chart represent many of the instances in the large upward spikes that appeared in the prior two line charts.

There is a cloud of mid-rated episodes with a large number of votes in the center of the plot; these represent the very earliest episodes in the series. There are no episodes which constitute low ratings and a high number of user votes. This is good, as that would indicate massive fan outrage (For comparison, the universally-panned [final season of Game of Thrones](https://www.ratingraph.com/tv-shows/game-of-thrones-ratings-26649/#votes) is a low-rated, high-voter scenario on the same website). The ratings division between filler and canon content is again observable here, as most of the lower-rated episodes constitute filler material.
...
We've seen that canonical episodes of the *One Piece* anime tend to receive higher ratings than filler episodes—by about 1 point on average. While that disparity isn't as evident when examining the total number of user ratings per episode, it's notable that episodes with exceptionally high numbers of votes are always canonical.

Moving forward, let's analyze these same metrics through the lens of narrative groupings. We'll compare the average ratings and total number of reviewer votes across episodes from different parts of the story.

#### Color map

Before creating any plots, we'll first establish a consistent color scheme for the data. We will primarily group by story saga for this section, and since all arcs are subcategories of some saga, it makes sense to color our data based on the 'Saga' category. I've established the following color assignments for each saga of the story, based on stylistic choices and themes present within each of them:
...
Continuing from the findings of the previous boxplot, we can observe that earlier story sagas like East Blue and Alabasta tend to receive the highest number of reviewer votes per episode (with East Blue leading by a considerable margin). As discussed earlier regarding the 'total_votes' line chart, these early "seasons" might have more reviews because the episodes have been aired for longer. Alternatively, it's possible that more individuals cease watching or reviewing episodes as time passes.

It’s also worth noting that the Punk Hazard and Fishman Island sagas arguably have the lowest overall popularity compared to other parts of the story; their low ratings and reviewer numbers place them in the bottom-left quadrant of the graph. Interestingly, the Dressrosa and Whole Cake Island sagas also have comparably low or lower reviewer numbers but maintain relatively high overall ratings. All of these sagas are set in the "New World" half of *One Piece*, which occurs after the time skip. Their lower reviewer levels are likely related to the sagas’ mid-to-late placement within the series' larger story structure.

Aside from these extreme examples, the Skypeia, Thriller Bark, and even Alabasta sagas sit near the center of the plotted data. These parts of the story might be considered to be of intermediate popularity. The Water Seven, Marineford, and Wano sagas all have similar reviewer levels but also received higher overall ratings. Notably, the fan-favorite Water Seven and Marineford sagas almost perfectly overlap in terms of ratings, reviewership, and episode count.
...
Let's take our previous saga bubble chart and break it down further by looking at individual arcs. In this updated version, each bubble will now represent a separate arc within the *One Piece* series. The size of each bubble will still reference the total number of episodes in that arc, and the color of each bubble will be consistent with the saga it belongs to, as per our earlier color mapping.

This visualization will allow us to compare arcs within a saga more directly, highlighting the differences in average ratings, reviewer votes, and episode counts across the various parts of the series.
...
We can see that major story arcs tend to be represented as larger bubbles toward the right side of the graph. This indicates that they generally consist of more episodes and have higher overall ratings. The Marineford arc is the highest-rated story arc, outperforming the overall ratings of the Marineford Saga as a whole. There is a clear ratings divide around the ~7.5 level, where bubbles on the right represent longer-form canonical story arcs, while bubbles on the left tend to represent shorter-form filler arcs. The major exceptions are the Fishman Island and Punk Hazard arcs, which underperform compared to other large canonical arcs. The shorter Levely Arc is the only "canon" arc with a lower rating than Punk Hazard and Fishman Island (* although, again, much of the content in Levely episodes could be considered "recap material").

Most arcs across all parts of the story tend to maintain reviewer levels in the average range of 100-200 votes per episode. The major exception to this is the collection of various East Blue arcs, most of which far exceed the average number of reviews per episode compared to arcs from any other part of the story. The first five story arcs of the series ("Romance Dawn", "Orange Town", "Syrup Village", "Baratie", and "Arlong Park") each receive hundreds more per-episode reviews on average than any other story arcs. We can also start to see the beginning of some layering here, where earlier story arcs "rest upon" their later counterparts due to decreasing reviewer levels over time; notice the blue and gold bubbles toward the higher voter levels, with a few pink Whole Cake Island bubbles sitting near the bottom.
...
This next plot changes the level of detail to individual episodes. It is effectively the same as the initial "Votes vs. Episode Rating" scatterplot, but with an alternate color scheme. The custom 'Saga' color scheme is applied in place of the orange/blue 'Function' color scheme. This allows for easier viewing of where individual episodes sit on the plot relative to other episodes, both from the same and different narrative sagas.
...
Developing on my previous comment, this plot seems to indicate more "layering" between 'total_votes' levels for different parts of the story. Some of this might be the layering order of the Plotly traces, but it does appear that the level of "Total Reviewer Votes" for earlier story sagas sits on the higher levels of the plot, while episodes from later story sagas rest toward the bottom, often in the chronological order of the 'Saga' field (with the somewhat exception of Wano). The discernible order of layers appears to be East Blue (blue) on top, followed by Alabasta (gold) and Skypeia (green); Whole Cake Island episodes (pink) tend to sit near the bottom, right below Dressrosa (orange). Episodes from all other *middle* sagas appear largely indiscernible or mixed together, sitting closer to the median line. One exception is the amount of reviewer vote levels for Wano episodes (dark purple); these appear all over the plot at various places and seem to be less clustered together.
...
Regarding reviewer activity, there seemed to be no significant differentiation between canon and filler episodes. Instead, we mostly saw reviewer activity change over time with regard to location in the overall narrative. Reviewer activity was initially very high at the start of the series before slowly declining, plateauing for the majority of the series, and fluctuating upwards again toward more recent material. This means that earlier storylines accumulated marginally more activity than later counterparts (with the exception of Wano). By a significant margin, the most-reviewed storylines were the first two: East Blue and Alabasta. The interaction between episode ratings and their respective viewer activity indicated a relatively stable overall satisfaction with the show, as there were no extreme instances of episodes with low ratings and high reviewer activity, indicating potential viewer displeasure or outrage.
